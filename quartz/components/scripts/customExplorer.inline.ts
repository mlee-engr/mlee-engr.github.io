...
class FileTrieNode {
  ...
  insert(path, file) {
    ...
    if (path.length === 1) {
      ...
      child.fileSegmentHint =
        fileParts[fileParts.length - path.length];
...
function renderTree(
  ...
  const folderTemplate =
    document.getElementById("template-folder");
  ...
  if (node.isFolder) {
    ...
    const folderContainer =
      clone.querySelector(".folder-container");

    let folderButton =
      clone.querySelector(".folder-button");

    const folderTitle =
      clone.querySelector(".folder-title");

    const folderOuter =
      clone.querySelector(".folder-outer");
    ...
    const isCollapsed =
      savedState[node.slug] !== undefined
        ? savedState[node.slug]
        : true;

    // If this folder is a prefix of the current
    // path, open it automatically.
    const simpleFolderPath =
      simplifySlug(node.slug);

    const folderIsPrefixOfCurrentSlug =
      simpleFolderPath &&
      simpleFolderPath ===
        simplifiedCurrentSlug.slice(
          0,
          simpleFolderPath.length
        );
    ...
    if (link) {
      // Use the actual Quartz slug from
      // the content index.
      link.href =
        resolveBasePath(
          node.data.slug
...
async function handleNavOrRender(e) {
  ...
  try {
    console.log(
      "[Explorer] Nav event received, generation:",
      thisGeneration
    );
    ...
    try {
      ...
      saved.forEach((item) => {
        savedState[item.path] =
          item.collapsed;
      });
    } catch (e) {
      console.error(
        "[Explorer] Error loading saved state:",
        e
      );
    }

    for (
      const explorer of allExplorers
    ) {
      const explorerUl =
      ...
      const dataFns =
        explorer.dataset.dataFns;

      const folderBehavior =
        explorer.dataset.behavior ||
        "collapse";

      // Build and render the tree
      console.log(
        "[Explorer] Starting tree build..."
      );
      ...
      // Check if another nav event started
      ...
      if (
        ...
        console.log(
          "[Explorer] Render generation is current, rendering tree"
        );

        console.log(
          "[Explorer] Trie result:",
          trie
            ? "success"
            : "null"
        );
        ...
        if (
          ...
          console.log(
            "[Explorer] Render complete, final list length:",
            explorerUl.children.length
          );
        } else {
          console.warn(
            "[Explorer] No trie or empty children"
          );
        }

        // Restore scrollTop position
        // or scroll to active element.
        const scrollTop =
        ...
        console.log(
          "[Explorer] Stale render generation, skipping tree render"
        );
      }

      // Always set up event listeners,
      // regardless of render generation.
      const cleanupHandlers = [];

      const explorerButtons =
        explorer.getElementsByClassName(
          "explorer-toggle"
        );

      for (
        const button of explorerButtons
      ) {
        const clickHandler =
          function () {
            const nearestExplorer =
              this.closest(
                ".explorer"
              );
            ...
            const explorerCollapsed =
              nearestExplorer.classList.toggle(
                "collapsed"
              );
        ...
        const iconClickHandler =
          function (evt) {
            ...
            const childFolderContainer =
              folderContainer.nextElementSibling;
            ...
            childFolderContainer.classList.toggle(
              "open"
            );

            const isCollapsed =
              !childFolderContainer.classList.contains(
                "open"
              );
        ...
        const buttonClickHandler =
          function (evt) {
            ...
            if (
              ...
              // When folderBehavior is "link",
              // the button has been replaced
              // with an <a> tag.
              //
              // Let the native link propagate
              // to Quartz's SPA router.
              return;
            } else {
              evt.stopPropagation();
    ...
    for (
      ...
      mobileExplorer.classList.remove(
        "hide-until-loaded"
      );
    ...
    console.error(
      "[Explorer] Fatal error in nav handler:",
      err
    );
  }
}
